    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (activeNetwork != null) { // connected to the internet
            if (activeNetwork.getType() == ConnectivityManager.TYPE_WIFI) {
                ...
                String bssid = getWifibssid(context);

                String name2 = getWifiName(context);
                Toast.makeText(context, "WIFI" + name + bssid, Toast.LENGTH_SHORT).show();
                ...
                for (AlarmItems curItem : allArray) {
                    ...
                    if (curItem.isCheck() == 1 && curItem.getEvent().equals("Specific WiFi Connected") && curItem.getKey().equals(bssid)) {

                        Log.d("Receiver", "Specific WiFi Connected");
                        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(context);
                        mBuilder.setSmallIcon(R.mipmap.ic_launcher);
                        mBuilder.setContentTitle(curItem.getRemind());
                        mBuilder.setContentText("Hey, you asked us to remind: " + curItem.getRemind());
